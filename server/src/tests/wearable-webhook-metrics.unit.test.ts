...
test("wearable webhook metrics aggregates status and provider counts", async () => {
  ...
  const snapshot = await getWearableWebhookAuditSnapshot(60);
  ...
  assert.ok(snapshot.source === "database" || snapshot.source === "memory_fallback");
});
